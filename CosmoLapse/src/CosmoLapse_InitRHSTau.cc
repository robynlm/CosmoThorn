...
static void CosmoLapse_InitRHSTau_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  #pragma omp parallel
  CCTK_LOOP3(CosmoLapse_InitRHSTau,
  ...
  CCTK_ENDLOOP3(CosmoLapse_InitRHSTau);
}
extern "C" void CosmoLapse_InitRHSTau(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_CosmoLapse_InitRHSTau
  DECLARE_CCTK_ARGUMENTS_CHECKED(CosmoLapse_InitRHSTau);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering CosmoLapse_InitRHSTau_Body");
  }
  if (cctk_iteration % CosmoLapse_InitRHSTau_calc_every != CosmoLapse_InitRHSTau_calc_offset)
  {
    return;
  }
  ...
  AssertGroupStorage(cctkGH, "CosmoLapse_InitRHSTau", 2, groups);
  ...
  LoopOverEverything(cctkGH, CosmoLapse_InitRHSTau_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving CosmoLapse_InitRHSTau_Body");
  }
}
