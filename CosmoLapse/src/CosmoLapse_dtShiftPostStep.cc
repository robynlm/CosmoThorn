...
extern "C" void CosmoLapse_dtShiftPostStep_SelectBCs(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_CosmoLapse_dtShiftPostStep_SelectBCs
  DECLARE_CCTK_ARGUMENTS_CHECKED(CosmoLapse_dtShiftPostStep_SelectBCs);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % CosmoLapse_dtShiftPostStep_calc_every != CosmoLapse_dtShiftPostStep_calc_offset)
    return;
...
static void CosmoLapse_dtShiftPostStep_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(CosmoLapse_dtShiftPostStep,
  ...
  CCTK_ENDLOOP3(CosmoLapse_dtShiftPostStep);
}
extern "C" void CosmoLapse_dtShiftPostStep(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_CosmoLapse_dtShiftPostStep
  DECLARE_CCTK_ARGUMENTS_CHECKED(CosmoLapse_dtShiftPostStep);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering CosmoLapse_dtShiftPostStep_Body");
  }
  if (cctk_iteration % CosmoLapse_dtShiftPostStep_calc_every != CosmoLapse_dtShiftPostStep_calc_offset)
  {
    return;
  ...
  const char* const groups[] = {
    ...
    "ML_BSSN::ML_Gammarhs",
    "ML_BSSN::ML_shiftrhs"};
  AssertGroupStorage(cctkGH, "CosmoLapse_dtShiftPostStep", 9, groups);
  
  switch (fdOrder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "CosmoLapse_dtShiftPostStep", 2, 2, 2);
      break;
    }
    
    case 4:
    {
      EnsureStencilFits(cctkGH, "CosmoLapse_dtShiftPostStep", 3, 3, 3);
      break;
    }
    
    case 6:
    {
      EnsureStencilFits(cctkGH, "CosmoLapse_dtShiftPostStep", 4, 4, 4);
      break;
    }
    
    case 8:
    {
      EnsureStencilFits(cctkGH, "CosmoLapse_dtShiftPostStep", 5, 5, 5);
      break;
    }
    default:
      CCTK_BUILTIN_UNREACHABLE();
  ...
  LoopOverInterior(cctkGH, CosmoLapse_dtShiftPostStep_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving CosmoLapse_dtShiftPostStep_Body");
  }
}
