...
static void CosmoLapse_RHSTau_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  CCTK_ENDLOOP3(CosmoLapse_RHSTau);
}
extern "C" void CosmoLapse_RHSTau(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_CosmoLapse_RHSTau
  DECLARE_CCTK_ARGUMENTS_CHECKED(CosmoLapse_RHSTau);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering CosmoLapse_RHSTau_Body");
  }
  if (cctk_iteration % CosmoLapse_RHSTau_calc_every != CosmoLapse_RHSTau_calc_offset)
  {
    return;
  }
  ...
  const char* const groups[] = {
    ...
    "ADMBase::metric",
    "ADMBase::shift",
    "CosmoLapse::Kthresholdrhs",
    "CosmoLapse::propertimerhs"};
  AssertGroupStorage(cctkGH, "CosmoLapse_RHSTau", 5, groups);
  ...
  LoopOverEverything(cctkGH, CosmoLapse_RHSTau_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving CosmoLapse_RHSTau_Body");
  }
}
