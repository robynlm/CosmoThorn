...
static void CosmoLapse_CTD_InitialTau_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  #pragma omp parallel
  CCTK_LOOP3(CosmoLapse_CTD_InitialTau,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL KtransitionL CCTK_ATTRIBUTE_UNUSED = KaTransition;
    /* Copy local copies back to grid functions */
    Ktransition[index] = KtransitionL;
    tau[index] = tauL;
  }
  CCTK_ENDLOOP3(CosmoLapse_CTD_InitialTau);
}
extern "C" void CosmoLapse_CTD_InitialTau(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_CosmoLapse_CTD_InitialTau
  DECLARE_CCTK_ARGUMENTS_CHECKED(CosmoLapse_CTD_InitialTau);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering CosmoLapse_CTD_InitialTau_Body");
  }
  if (cctk_iteration % CosmoLapse_CTD_InitialTau_calc_every != CosmoLapse_CTD_InitialTau_calc_offset)
  {
    return;
  }
  ...
  AssertGroupStorage(cctkGH, "CosmoLapse_CTD_InitialTau", 2, groups);
  ...
  LoopOverEverything(cctkGH, CosmoLapse_CTD_InitialTau_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving CosmoLapse_CTD_InitialTau_Body");
  }
}
