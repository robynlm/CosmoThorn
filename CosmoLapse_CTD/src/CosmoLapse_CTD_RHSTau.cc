...
static void CosmoLapse_CTD_RHSTau_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  #pragma omp parallel
  CCTK_LOOP3(CosmoLapse_CTD_RHSTau,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL taurhsL CCTK_ATTRIBUTE_UNUSED = 
      pow(fabs(-2*betaxL*(betayL*gxyL + betazL*gxzL) - 2*betayL*betazL*gyzL + 
      pow(alpL,2) - gxxL*pow(betaxL,2) - gyyL*pow(betayL,2) - 
      gzzL*pow(betazL,2)),0.5);
    
    CCTK_REAL KtransitionrhsL CCTK_ATTRIBUTE_UNUSED = 0;
    /* Copy local copies back to grid functions */
    Ktransitionrhs[index] = KtransitionrhsL;
    taurhs[index] = taurhsL;
  }
  CCTK_ENDLOOP3(CosmoLapse_CTD_RHSTau);
}
extern "C" void CosmoLapse_CTD_RHSTau(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_CosmoLapse_CTD_RHSTau
  DECLARE_CCTK_ARGUMENTS_CHECKED(CosmoLapse_CTD_RHSTau);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering CosmoLapse_CTD_RHSTau_Body");
  }
  if (cctk_iteration % CosmoLapse_CTD_RHSTau_calc_every != CosmoLapse_CTD_RHSTau_calc_offset)
  {
    return;
  }
  ...
  const char* const groups[] = {
    ...
    "ADMBase::metric",
    "ADMBase::shift",
    "CosmoLapse_CTD::Kthresholdrhs",
    "CosmoLapse_CTD::propertimerhs"};
  AssertGroupStorage(cctkGH, "CosmoLapse_CTD_RHSTau", 5, groups);
  ...
  LoopOverEverything(cctkGH, CosmoLapse_CTD_RHSTau_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving CosmoLapse_CTD_RHSTau_Body");
  }
}
