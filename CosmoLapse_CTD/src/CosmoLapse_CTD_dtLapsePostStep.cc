...
extern "C" void CosmoLapse_CTD_dtLapsePostStep_SelectBCs(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_CosmoLapse_CTD_dtLapsePostStep_SelectBCs
  DECLARE_CCTK_ARGUMENTS_CHECKED(CosmoLapse_CTD_dtLapsePostStep_SelectBCs);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % CosmoLapse_CTD_dtLapsePostStep_calc_every != CosmoLapse_CTD_dtLapsePostStep_calc_offset)
    return;
  ...
  ierr = KrancBdy_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "CosmoLapse_CTD::Kthreshold","flat");
  if (ierr < 0)
    CCTK_WARN(CCTK_WARN_ALERT, "Failed to register flat BC for CosmoLapse_CTD::Kthreshold.");
...
static void CosmoLapse_CTD_dtLapsePostStep_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(CosmoLapse_CTD_dtLapsePostStep,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL Kth CCTK_ATTRIBUTE_UNUSED = -2.*pow(tauL,-1);
    ...
    CCTK_REAL Kb CCTK_ATTRIBUTE_UNUSED = IfThen(KbExpression == 1,Kth,0.);
  ...
  CCTK_ENDLOOP3(CosmoLapse_CTD_dtLapsePostStep);
}
extern "C" void CosmoLapse_CTD_dtLapsePostStep(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_CosmoLapse_CTD_dtLapsePostStep
  DECLARE_CCTK_ARGUMENTS_CHECKED(CosmoLapse_CTD_dtLapsePostStep);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering CosmoLapse_CTD_dtLapsePostStep_Body");
  }
  if (cctk_iteration % CosmoLapse_CTD_dtLapsePostStep_calc_every != CosmoLapse_CTD_dtLapsePostStep_calc_offset)
  {
    return;
  ...
  const char* const groups[] = {
    ...
    "ADMBase::lapse",
    "ADMBase::shift",
    "CosmoLapse_CTD::Kthreshold",
    "ML_BSSN::ML_lapserhs",
    "ML_BSSN::ML_trace_curv",
    "CosmoLapse_CTD::propertime"};
  AssertGroupStorage(cctkGH, "CosmoLapse_CTD_dtLapsePostStep", 7, groups);
  
  switch (fdOrder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "CosmoLapse_CTD_dtLapsePostStep", 2, 2, 2);
      break;
    }
    
    case 4:
    {
      EnsureStencilFits(cctkGH, "CosmoLapse_CTD_dtLapsePostStep", 3, 3, 3);
      break;
    }
    
    case 6:
    {
      EnsureStencilFits(cctkGH, "CosmoLapse_CTD_dtLapsePostStep", 4, 4, 4);
      break;
    }
    
    case 8:
    {
      EnsureStencilFits(cctkGH, "CosmoLapse_CTD_dtLapsePostStep", 5, 5, 5);
      break;
    }
    default:
      CCTK_BUILTIN_UNREACHABLE();
  ...
  LoopOverInterior(cctkGH, CosmoLapse_CTD_dtLapsePostStep_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving CosmoLapse_CTD_dtLapsePostStep_Body");
  }
}
